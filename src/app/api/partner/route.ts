...
export async function POST(request: NextRequest) {
  ...
  const {name, company, email, phone, market, message, honeypot} = body;
  ...
  const telegramText = `[ПАРТНЕР] Нова заявка

Ім'я: ${name}
Компанія: ${company}
Email: ${email}
${phone ? `Телефон: ${phone}` : ''}
Ринок: ${market || 'Не вказано'}
${message ? `Повідомлення: ${message}` : ''}`;

  const emailHtml = `
<h2>Нова заявка від партнера</h2>
<p><strong>Ім'я:</strong> ${name}</p>
<p><strong>Компанія:</strong> ${company}</p>
<p><strong>Email:</strong> ${email}</p>
${phone ? `<p><strong>Телефон:</strong> ${phone}</p>` : ''}
<p><strong>Ринок:</strong> ${market || 'Не вказано'}</p>
...
`;
  ...
  const results = await Promise.allSettled([
    ...
    resend.emails.send({
      ...
      subject: `[SafeEra ПАРТНЕР] Нова заявка — ${name} (${company})`,
      html: emailHtml,
    }),
  ]);
