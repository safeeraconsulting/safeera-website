...
export default function PartnerForm() {
  const t = useTranslations('formPartner');
  ...
  return (
    <section id="partner-form-section" className="bg-forest text-off-white py-[clamp(72px,9vw,140px)] px-[clamp(20px,5vw,80px)]">
      <div className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-[clamp(40px,6vw,88px)]">
        <div>
          <div className="eyebrow text-lime mb-[22px]">{ts('partnerFormEyebrow')}</div>
          ...
          <div className="flex flex-col gap-2 text-sm opacity-70">
            ...
            <a href="https://linkedin.com/company/safeera-property-consulting" target="_blank" rel="noopener noreferrer" className="hover:opacity-100 transition-opacity">linkedin/safeera</a>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[11px] tracking-[0.18em] uppercase font-medium">{t('companyLabel')}</label>
              <input type="text" name="company" required placeholder={t('companyPh')}
                className="w-full px-0 py-3 bg-transparent border-b border-off-white/20 text-base font-light text-off-white focus:border-off-white focus:outline-none transition-colors placeholder:text-off-white/30" />
            </div>
          </div>
